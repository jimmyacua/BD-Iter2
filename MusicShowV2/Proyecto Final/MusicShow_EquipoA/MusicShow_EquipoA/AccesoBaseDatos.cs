...
namespace Lab_Interfaces
{
    class AccesoBaseDatos
    {
        ...
        //String conexion = "Data Source=10.1.4.55;User ID=username;Password=password; Initial Catalog=dabatase_name; Integrated Security=false";
        ...
        public DataTable FiltrarCancion(String nombre)
        {
            using (SqlConnection con = new SqlConnection(conexion))
            {
                using (SqlCommand cmd = new SqlCommand("SELECT * from Repertorio where NombreAn = '" + nombre +"';", con))
                {
                    try
                    {
                        SqlParameter paramNombre = new SqlParameter("@nombre",
