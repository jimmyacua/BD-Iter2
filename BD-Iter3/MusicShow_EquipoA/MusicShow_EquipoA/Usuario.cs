...
namespace MusicShow_EquipoA
{
    public class Usuario
    {
        ...
        public int AgregarBuscador(string nombre, string email, string provincia, string canton)
        {
            String insertar = "exec agregarBuscador @nombre = '" + nombre + "', @email = '" + email +  "', @provincia = '" + provincia + "', @canton = '" + canton + "';";
            return bd.ActualizarDatos(insertar);
        }
        ...
        public int VerificarUsuario(String nombre) {
            ...
            if (con.HasRows) { //arreglar esto
                return 1;
            }
            return 0;
        }

        public int VerificarUsuarioBuscador(String nombre)
        {
            String consulta = "exec consultarBuscadorNombre @nombreBuscador = '" + nombre + "';";
            SqlDataReader con = bd.EjecutarConsulta(consulta);
            if (con.HasRows)
            { //arreglar esto
                return 1;
        ...
        public DataTable obtenerCanciones(string filtroNombre)
        {
            DataTable tabla = null;
            try
            {
            
                tabla = bd.FiltrarCancion(filtroNombre);
