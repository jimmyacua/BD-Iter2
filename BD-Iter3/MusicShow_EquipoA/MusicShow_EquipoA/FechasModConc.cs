...
namespace MusicShow_EquipoA
{
    public partial class FechasModConc : MetroForm
    {
        ...
        private void botonElm_Click_1(object sender, EventArgs e)
        {
            ...
            nombreAn = gridFechas.Rows[gridFechas.CurrentRow.Index].Cells[2].Value.ToString();
            nombreConc = gridFechas.Rows[gridFechas.CurrentRow.Index].Cells[1].Value.ToString();
            fechaG = gridFechas.Rows[gridFechas.CurrentRow.Index].Cells[0].Value.ToString();

            //MessageBox.Show(fechaG, "Modificar Repertorio", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);

            bd.ActualizarDatos("delete from Fecha_Hora where NombreAn = '" + modConc.nombreAn + "' and NombreConcierto = '" + modConc.nombreConcierto + "' and Fecha_Hora = '" + fechaG + "';");
            LlenarTabla(gridFechas, "select f.Fecha_Hora as Fecha_y_Hora, f.NombreConcierto as Nombre_Concierto, f.NombreAn as Nombre_Anunciante from Fecha_Hora f where f.NombreAn = '" + modConc.nombreAn + "' and f.NombreConcierto = '" + modConc.nombreConcierto + "'; ");
            
            if (fechaG != "")
            {
                string[] fH = fechaG.Split(' ');
                string fechYHor = fH[0] + " " + fH[1] + " " + fH[2];// + fH[4];
                string[] fecha = fH[0].Split('/');
                string f = fecha[2] + "-" + fecha[1] + "-" + fecha[0];
                string[] hora = fH[1].Split(':');
                if (fH[2].Contains("p")) {
