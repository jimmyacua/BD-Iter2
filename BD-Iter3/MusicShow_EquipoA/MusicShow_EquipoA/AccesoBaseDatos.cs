...
namespace Lab_Interfaces
{
    class AccesoBaseDatos
    {
        ...
        String conexion = "Data Source=10.1.4.55;User ID=B50060;Password=Saprissa.14; Initial Catalog=DB_EQUIPO_A; Integrated Security=false";
        ...
        public DataTable FiltrarCancion(String nombre)
        {
            using (SqlConnection con = new SqlConnection(conexion))
            {
                using (SqlCommand cmd = new SqlCommand("Select r.NombreAn, r.NombreCancion as Anunciante, r.NombreInterprete as Intérprete, c.Año, i.NombreIdioma as Idioma, g.NombreGenero as Género from Repertorio r join Cancion c on r.NombreCancion = c.Nombre join Idioma i on i.NombreIdioma = c.NombreIdioma join Genero g on g.NombreGenero = c.NombreGenero where NombreAn = '" + nombre +"';", con))
                {
                    try
                    {
                        SqlParameter paramNombre = new SqlParameter("@nombre",
