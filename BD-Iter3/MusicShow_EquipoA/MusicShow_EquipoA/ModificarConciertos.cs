...
namespace MusicShow_EquipoA
{
    public partial class ModificarConciertos : MetroForm
    {
        ...
        private void metroComboBox2_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            SqlDataReader r = bd.EjecutarConsulta("select Descripcion from Concierto where NombreConcierto = '" + nombreConcierto + "' and NombreAn = '" + nombreAn + "';");

            while (r.Read())
            {
                descr = r.GetString(0); //The 0 stands for "the 0'th column", so the first column of the result.
                                        // Do somthing with this rows string, for example to put them in to a list
            }



            r = bd.EjecutarConsulta("select Cupo from Concierto where NombreConcierto = '" + nombreConcierto + "' and NombreAn = '" + nombreAn + "';");
            while (r.Read())
            {
                cup = r.GetInt32(0); //The 0 stands for "the 0'th column", so the first column of the result.
                                     // Do somthing with this rows string, for example to put them in to a list
            }

            cupo.Text = cup.ToString();

            r = bd.EjecutarConsulta("select NombreLugar from Concierto where NombreConcierto = '" + nombreConcierto + "' and NombreAn = '" + nombreAn + "';");
            while (r.Read())
            {
                lug = r.GetString(0); //The 0 stands for "the 0'th column", so the first column of the result.
                                      // Do somthing with this rows string, for example to put them in to a list
            }

            descripcion.Text = descr;
            nombre.Text = nombreConcierto;
            comboLugar.SelectedIndex  =comboLugar.FindStringExact(lug);
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            if (nombConc.Text != "Seleccione")
            {
                ...
                bd.ActualizarDatos("update Concierto set NombreConcierto = '"+nombreConcierto+"', Cupo = "+cup+", Descripcion ='"+ descr+"', NombreLugar ='"+lug+ "' where NombreConcierto = '" + nombConc.Text + "' and NombreAn = '" + menu.nombreAn + "'");
