...
namespace MusicShow_EquipoA
{
    public partial class PaginaInicio : MetroForm
    {
        ...
        username = TX_USERNAME.Text;
            ...
            if (radioAnunciante.Checked == true)
            {
                ...
                if (valido == 1)
                {
                    ...
                    read = bd.EjecutarConsulta("select count(*) from Me_Gustas m where m.NombreAn = '" + username + "'");

                    while (read.Read())
                    {
                        likes = read.GetInt32(0); //The 0 stands for "the 0'th column", so the first column of the result.
                                                  // Do somthing with this rows string, for example to put them in to a list
                    }

                    MenuAnunciante menAn = new MenuAnunciante();
                    menAn.setInformacion(username, myString, likes.ToString());
                    menAn.Show();
                    Hide();
                }
                else
            ...
            else if (radioBuscador.Checked == true)
            {
                int valido = user.VerificarUsuarioBuscador(username);
                if (valido == 1)
                {
                 
                    MenuBuscador meBus = new MenuBuscador(username);
                    meBus.setInformacion(username);
                    meBus.Show();
                    Hide();
                }
                else
            ...
            {
                MessageBox.Show("Seleccione si es anuncante o buscador", "ERROR", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        ...
        {
            CrearPerfilDeBuscador crearPer = new CrearPerfilDeBuscador();
            crearPer.Show();
            Hide();
        }
    }
